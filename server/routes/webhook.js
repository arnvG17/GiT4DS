...
  // Total commits per user (Highest count first: -1)
...
  // Earliest commit per user (Latest first: -1)
  // Note: This aggregates the MINIMUM (earliest) commit time, but ranks users
  // who achieved that first commit most RECENTLY at the top.
  const firstCommits = await Commit.aggregate([
    { $group: { _id: "$userId", firstCommitTimestamp: { $min: "$timestamp" } } },
    { $sort: { firstCommitTimestamp: -1 } } // CHANGED: Sorting by -1 to show latest first
  ]);
...
  // Recent activity (Latest first: -1)
